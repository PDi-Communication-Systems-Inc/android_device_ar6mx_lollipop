...
if [ $COUNT -ge "1" ];
  then
      ...
      log -p i -t "$TAG" "$COUNT Atmel Touchscreen discovered"
      ...
      if [ $RESULT -gt "0" ];
         ...
         while [ $COUNTER -lt 5 ]; do
            log -p e -t "$TAG" "Reset failed with result code $RESULT ... Retrying mxt-app reset, try $COUNTER"
...
if [ $COUNT -ge "1" ];
   then
       log -p i -t "$TAG" "$COUNT eGalax Touchscreen discovered, resetting usb controller"
       ...
       log -p i -t "$TAG" "Resetting touchscreen device"
       ...
       if [ $DEVICEID == "a04d" ];
          then
           ...
           then
              log -p i -t "$TAG" "$DEVICEID does not like to reset after bus, not doing reset"
       else
              log -p i -t "$TAG" "Unknown EGalax touchscreen $DEVICEID not doing reset"
       fi
...
log -p i -t "$TAG" "Result of i2c detection is $I2CDETECTCMD"
...
if [ $I2CDETECTCMD == "4a" ];
   then
      ...
      if [ $RESULT -ne "0" ];
      then
         log -p e -t "$TAG" "Driver failed to load properly, trying to remove"
      ...
      log -p i -t "$TAG" "found touchscreen $NAME"
   ...
   log -p w -t "$TAG" "i2c touchscreen not present"
